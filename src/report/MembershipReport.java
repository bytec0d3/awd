...
public class MembershipReport extends Report implements UpdateListener {
    ...
    public void updated(List<DTNHost> hosts) {
        ...
        if(!isWarmup()){
            ...
            for(DTNHost h : hosts){
                ...
                membershipGroup.update(((AutonomousHost)h).getGroup());
                nodesMembership.put(h, membershipGroup);

                //=================================================================

                /*AutonomousHost autonomousHost = (AutonomousHost)h;

                List<String> reachableNodes = new ArrayList<>();
                if(autonomousHost.getGroup() != null){
                    for(DTNHost rh : autonomousHost.getGroup()){
                        reachableNodes.add(rh.name);
                    }
                }

                write(String.format("%.2f", SimClock.getTime()) + " " + autonomousHost.name + " " + Arrays.toString(reachableNodes.toArray()));*/
            }
        }
    }
    ...
    private class MembershipGroup{
        ...
        public void update(Set<DTNHost> currentGroup){

            if(currentGroup == null) currentGroup = new HashSet<>();
