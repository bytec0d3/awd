		...
		if (drawCoverage && node.isRadioActive()) {
			...
			for (NetworkInterface ni : interfaces) {
				...
				if(node instanceof AutonomousHost
						&& ((AutonomousHost) node).getCurrentStatus() == AutonomousHost.HOST_STATUS.OFFLINE){

					g2.setPaint(new Color(229,229,229,100));
					g2.fill(coverage);

				}else if(node instanceof AutonomousHost){

					g2.setPaint(GroupColors.getInstance().getGroupColor(((AutonomousHost) node).getGroupName()));
					g2.fill(coverage);

				}

				//g2.setPaint(Color.RED);
				//g2.setPaint(GroupColors.getInstance().getGroupColor(node.getGroupId()));
				//g2.fill(coverage);
				g2.draw(coverage);
			}
		}
