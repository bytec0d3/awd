...
import awd.SimpleAutonomousHost;
...
public class SimScenario implements Serializable {
	...
	protected SimScenario() {
		...
		if(s.contains(HOST_TYPE)) this.hostType = s.getSetting(HOST_TYPE);
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				if(this.hostType != null && this.hostType.compareTo("CompleteAutonomousHost") == 0)
					host = new CompleteAutonomousHost(this.messageListeners, this.movementListeners, gid, interfaces, comBus,
							mmProto, mRouterProto);
				else if(this.hostType != null && this.hostType.compareTo("SimpleAutonomousHost") == 0)
					host = new SimpleAutonomousHost(this.messageListeners, this.movementListeners, gid, interfaces, comBus,
						mmProto, mRouterProto);
				else
					// prototypes are given to new DTNHost which replicates
					// new instances of movement model and message router
					host = new DTNHost(this.messageListeners, this.movementListeners,	gid, interfaces, comBus,
							mmProto, mRouterProto);
